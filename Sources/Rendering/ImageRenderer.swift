...
final class ImageRenderer {
  ...
  func draw(
    ...
    program.setVec4(
      "uTint",
      value: (Float(tintColor.red), Float(tintColor.green), Float(tintColor.blue), Float(tintColor.alpha) * opacity))
  ...
  /// Draw the image scaled to a target size in pixels at (x, y).
