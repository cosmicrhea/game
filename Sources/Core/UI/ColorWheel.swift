@MainActor
public final class ColorWheel: OptionsControl {
  // MARK: - Public API
  ...
  // Shared wheel image cache - all color wheels use the same image
  private static var sharedWheelImage: Image? = nil
  private static var sharedWheelPixelSize: Int = 0
  ...
  public func draw() {
    ...
    // Use shared wheel image - only generate once for all color wheels
    let pixelSize = max(8, Int(size.rounded()))
    if Self.sharedWheelImage == nil || Self.sharedWheelPixelSize != pixelSize {
      Self.ensureSharedWheelImage(diameterPx: pixelSize)
    }

    if let image = Self.sharedWheelImage {
  ...
  private static func ensureSharedWheelImage(diameterPx: Int) {
    sharedWheelPixelSize = diameterPx
    ...
    for y in 0..<h {
      ...
      for x in 0..<w {
        ...
        if inside {
          ...
          let (rr, gg, bb) = hsvToRgb(hue, sat, 1.0)
    ...
    sharedWheelImage = Image.uploadToGL(pixels: pixels, width: w, height: h, pixelScale: 1.0)
  }
