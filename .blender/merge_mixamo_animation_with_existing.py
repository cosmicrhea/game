...
def main():
	global existing_actions

	# existing_actions = bpy.data.actions
	existing_actions = [a.name for a in bpy.data.actions]
	# filepaths = glob("/Users/Freya/Downloads/mixamo_workspace/goth_girl/*.fbx")
	# filepaths.sort()
	# pprint(filepaths)
	# filepaths = ["/Users/freya/Downloads/Femme Peek Around Corner.fbx"]
	filepaths = glob("/Users/Freya/Downloads/mixamo_workspace/nils/new/*.fbx")
	filepaths.sort()
	...
	for (i, filepath) in enumerate(filepaths):
		...
		new_action = next(a for a in bpy.data.actions if "|mixamo.com|" in a.name)
		print("new_action =", new_action)
		new_action.name = Path(filepath).stem
...
def add_root_bone():
	...
	armature = [a for a in bpy.data.objects if isinstance(a.data, bpy.types.Armature)][-1]
	...
	if armature.data.edit_bones.keys():
