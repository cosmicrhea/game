...
void main() {
  ...
  // CutoutCoverage falls through to PBR shading after discard handling.
  ...
  vec3 F0 = (useAlphaHash && alphaMode == 2) ? vec3(0.0) : vec3(0.04);
  ...
  // Cutout pass: keep alpha for alpha-to-coverage, otherwise output opaque
  float outputAlpha = useAlphaToCoverage ? finalAlpha : 1.0;
