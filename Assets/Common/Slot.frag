...
void main() {
    ...
    float borderInner = 1.0 - smoothstep(uBorderThickness - 1.0, uBorderThickness, distance);
    float borderMask = borderOuter - borderInner;
    
    // Inner shadow - create inset effect
    float innerShadowDistance = roundedBoxSDF(uv - center, halfSize - vec2(4.0), uCornerRadius - 2.0);
