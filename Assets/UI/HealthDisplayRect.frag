// Engine-compatible variant: uses GLScreenEffect passthrough vertex which
// outputs TexCoord
#version 330 core
in vec2 TexCoord;
out vec4 FragColor;
...
uniform float
    uThickness;       // line thickness multiplier (in screen-space via fwidth)
...
uniform float
    uSpikeThreshold; // only positive excursions above this create spikes (0..1)
...
void main() {
  ...
  // rectangular band with spikes (reuse existing an)
  float n2 = noise(vec2(an * uSpikeFreq, iTime * 0.7));
  float spike = max(0.0, (n2 - uSpikeThreshold)) * uSpikeAmp;

  vec2 d = abs(p) - vec2(uRadius);
  vec2 dir = normalize(p);
  d -= dir * spike * uSpikeLen;
  float outside = max(d.x, d.y);
  float band = abs(outside);
  ...
  // Color by health bands: OK (cyan) / Caution (amber) / Fatal (rose)
  vec3 baseCyan = vec3(0.68, 0.90, 0.97);
  vec3 amber = vec3(0.9608, 0.6196, 0.0431);
  vec3 rose = vec3(0.9569, 0.2471, 0.3686);
  vec3 ringColor = baseCyan;
  if (health < 0.50 && health >= 0.33)
    ringColor = amber;
  if (health < 0.33)
    ringColor = rose;
  vec3 color = ringColor * ring;
  ...
  // inner haze based on rectangular band
  float innerMask = smoothstep(0.3, 0.0, band);
  ...
  vec3 outRGB = darkenedBase + innerColor + color * ringAlpha +
                mix(vec3(0.0), baseCyan, 0.7) * glow;
